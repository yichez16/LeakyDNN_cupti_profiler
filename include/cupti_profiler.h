...
//        CUresult _status = cuCtxCreate(&m_context, 0, m_device);
//        if (_status != CUDA_SUCCESS) {
...
//      cuCtxGetCurrent(&m_context);
      ...
      for(int i = 0; i < m_num_metrics; ++i) {
        // std::cout<< m_metric_names[i].c_str()<<std::endl;
      ...
      for(int i = 0; i < m_num_events; ++i) {
        // std::cout<< m_event_names[i].c_str()<<std::endl;
      ...
      for(auto &k: m_kernel_data) {
        ...
        for(int i = m_metric_passes;
          ...
          for(int j = 0; j < data[i].num_events; ++j) {
            // std::cout<<data[i].event_values[j]<<std::endl;
